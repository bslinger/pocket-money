...
import { PlusCircle, Check, CheckCheck, X, LogOut, TrendingUp, Plus, Minus, Undo2, Split, Trash2 } from 'lucide-react';
...
type SplitAllocation = { id: string; accountId: string; percentage: number };

function SplitAccountSelector({
  accounts,
  allocations,
  totalAmount,
  currencySymbol,
  ...
  return (
    <div className="space-y-2">
      {allocations.map((alloc, idx) => {
        const amt = totalAmount > 0
          ? (idx === allocations.length - 1
              ? totalAmount - allocations.slice(0, -1).reduce((s, a) => s + Math.floor(totalAmount * a.percentage / 100 * 100) / 100, 0)
          ...
          <div key={alloc.id} className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <input
                ...
                onChange={e => updatePct(alloc.id, parseFloat(e.target.value) || 0)}
            ...
            {totalAmount > 0 && (
              <span className="text-xs text-bark-400 w-14 text-right shrink-0">
                {currencySymbol}{amt.toFixed(useIntegers ? 0 : 2)}
              </span>
            )}
            {allocations.length > 2 && (
              <button type="button" onClick={() => remove(alloc.id)} className="text-bark-400 hover:text-redearth-400 transition-colors shrink-0">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs font-medium ${isValid ? 'text-gumleaf-500' : 'text-redearth-400'}`}>
...
function QuickTransactionModal({
  ...
  const [splitAllocations, setSplitAllocations] = useState<SplitAllocation[]>(() => [
    ...
    { id: crypto.randomUUID(), accountId: accounts[1]?.id ?? accounts[0]?.id ?? '', percentage: 50 },
  ]);
  ...
  function submit(e: React.FormEvent) {
    ...
    if (type === 'credit' && isSplit) {
      // Build split amounts — last row gets remainder to absorb rounding
      const splits = splitAllocations.map((alloc, idx) => {
        const amt = idx === splitAllocations.length - 1
          ? totalAmount - splitAllocations.slice(0, -1).reduce((s, a) => s + Math.floor(totalAmount * a.percentage / 100 * 100) / 100, 0)
          : Math.floor(totalAmount * alloc.percentage / 100 * 100) / 100;
        return { account_id: alloc.accountId, amount: amt };
      });

      router.post(route('transactions.split'), {
  ...
  function switchType(newType: 'credit' | 'debit') {
    ...
    if (newType === 'debit') setIsSplit(false);
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
          ...
          {/* Account selector — below Note, only when relevant */}
          ...
          <Button
            type="submit"
            disabled={processing || !data.amount || (type === 'credit' && isSplit && !splitValid)}
